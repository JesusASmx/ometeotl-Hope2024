...
train = pd.read_csv("..//Dataset//PolyES_train.csv")
val = pd.read_csv("..//Dataset//PolyES_val.csv")
train_df = pd.concat([train, val])

print(train_df["binary"].value_counts())

test_df = pd.read_csv("..//Dataset//Hope_test/PolyES_test.csv")
...
batch_size = 32
max_length = 128
labels_column = "binary"
labels_ids = {'Not Hope': 0, 'Hope': 1}
...
def validation(dataloader, device_):
    ...
    for batch in tqdm(dataloader, total=len(dataloader)):
        ...
        with torch.no_grad():
            outputs = model(**batch)
            loss, logits = outputs[:2]
            #logits es <class 'torch.Tensor'> y tiene shape [16,2]
            logits = logits.detach().cpu().numpy()
            total_loss += loss.item()
    ...
    for i in range(folds):
        try:
            ...
            if folds == 1:
                ...
                entrena = pd.read_csv("..//Dataset//PolyES_train.csv")
                validac = pd.read_csv("..//Dataset//PolyES_val.csv")
...
for x in tqdm(range(len(test_ids)), desc='###PREDICTING OVER THE TEST SET###'):
    ...
    predict_content = logits.argmax(axis=-1).flatten().tolist()
    test_pred = predict_content[0] # Relabed moved to EDI_submission.
    final_predictions.append(test_pred)
    #print(f"ID: {test_ids[x]}, SAMPLE: {logits} PREDICTION: {test_pred}")

resuls = submission(final_predictions)
...
ids_labels = {0: 'Not Hope', 1:'Hope'}

elected = sorted_nhs #Tiene la mayor cantidad de samples.
elegido = 0 #Not Hope

#Los 788 con mayor probabilidad de pertenecer a la clase:
elegidos = list(elected)[-788:]
